...
import {EnterpriseUserDTO} from '../../models/enterprise-user';
  ...
  implements OnInit
{
  ...
  onEdit(rowData: EnterpriseUserDTO): void {
    this.selected = [rowData];
    const userId = this.selected[0].id; // Retrieve the selected user's ID.
    void this.router.navigate([
      `/${AppRoutingConstants.AUTH_PATH}/${AppRoutingConstants.UPDATE_USER_PATH}`,
      userId
    ]);
  }
