...
public class AuthenticatorServiceImpl implements AuthenticatorService {
    ...
    public UserEntity authenticate(CredentialsVerification verification) {
        try {
            UserAuthenticator userAuthenticator = repository.findByCredentialId(verification.id()).orElseThrow(() -> new BadCredentialsException("Unknown credentials"));
    ...
    private UserAuthenticator createSerializedAuthenticator(CredentialsRegistration registration, UserEntity user) {
        return UserAuthenticator.builder()
                .credentialId(registration.credentials().id())
                .user(user)
                .credentialName(registration.name())
                .attestationObject(Base64.getUrlDecoder().decode(registration.credentials().response().attestationObject()))
                .signCount(0)
                .build();
    }
}
